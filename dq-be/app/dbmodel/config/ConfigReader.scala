...
import dbmodel.ModelUtils
import dbmodel.ModelUtils._
import dbmodel.checks._
import dbmodel.metrics._
import dbmodel.sources.Source.SourceType
import dbmodel.sources._
...
object ConfigReader {
  ...
  def parseConfiguration(configFile: File): Option[List[String]] = {
    ...
    val vsQueue: Queue[(String, (Source, VirtualSourceDB))] = parseVirtualSources(configObj)
  ...
  def parseVirtualSources(configObj: Config): Queue[(String, (Source, VirtualSourceDB))] = {

    val sourceList = configObj.getObjectList("VirtualSources").toList
    val vsMap: Map[String, (Seq[String], (Source, VirtualSourceDB))] = sourceList.map { sc =>
      val generalConfig = sc.toConfig
      ...
      val vs = new VirtualSourceDB(id, tipo, left, right, query)
      id -> (parents, (bs, vs))
    }.toMap
