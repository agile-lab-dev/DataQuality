...
/**
  ...
  *
  * Manager to do operations with local SQLite database.
  * Probably, we'll move to other storing platform in future
  */
class LocalDBManager(settings: DQSettings) extends Logging {

  lazy private val dbFilePath: DatabaseConfig =
    settings.resStorage.asInstanceOf[DatabaseConfig]
  lazy private val connection = dbFilePath.getConnection
  ...
  def saveResultsToDB(metrics: Seq[AnyRef], tb: String): Unit = {
    val table = utils.makeTableName(dbFilePath.schema, tb)
          ...
          .foreach(f => {
            ...
            value match {
              ...
              case x: Seq[_] =>
                ...
                val array: Array = connection.createArrayOf("text", xs.toArray)
