...
if [[ ! -z $SUMMARY_DIR ]]; then
  ...
  if (( filesize < MAX_SIZE )); then
    printf "$TEXT" | mail -s "[$STATUS] $APP_NAME Data Quality report" -a $SUMMARY_DIR/out.zip -r ${MAIL_FROM} ${MAIL_TO}
    rm $SUMMARY_DIR/out.zip
    exit 0
  fi
fi

printf "$TEXT" | mail -s "[$STATUS] $APP_NAME Data Quality report" -r ${MAIL_FROM} ${MAIL_TO}
exit 0
